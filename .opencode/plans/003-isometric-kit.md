...
8. Loot: enemies drop a collectible item on death (`enemy_mover.die()`); items support auto-pickup on contact and key-based pickup, with optional despawn timers. Shot enemies drop coins in the combined sandbox, and the player collects them (coin counter in the HUD; each pickup adds its value to the wallet).
...
- `scenes/loot_coin.tscn` — `loot_item` variant: flat gold cylinder coin named "Coin" (used by the combined sandbox so enemy kills drop currency).
...
- `combined_sandbox.tscn` — player + configurable wave spawner + shooting + loot + four pre-defined Rapid tower build pads all together. Configure spawn/rally points (click to place), wave count, and enemies per wave via the panel (1–5, then ∞); Start Waves, then shoot with Space/Fire (auto-fire and fire rate controls included). In ∞ mode the spawner ramps from 1 enemy per wave, +1 every N waves (configurable). Shots kill enemies permanently and drop coins worth currency (picked up on contact; wallet starts at 0), which pays the four build pads that build/upgrade functional Rapid towers (stages `[15, 30, 50]`); waves refill the field.
